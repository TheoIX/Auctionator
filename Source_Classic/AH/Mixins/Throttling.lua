...
function AuctionatorAHThrottlingFrameMixin:OnEvent(eventName, ...)
  ...
  elseif eventName == "NEW_AUCTION_UPDATE" then
    ...
    -- 2.5.3/private-core compatibility patch:
    -- Auctionator 9.2.31 waits for AUCTION_OWNED_LIST_UPDATE after posting.
    -- Some servers do not reliably fire that owned-list event after a successful
    -- post, which causes a long hitch and the "server took too long" warning.
    -- Treat NEW_AUCTION_UPDATE as enough proof that the post went through.
  ...
  elseif eventName == "AUCTION_MULTISELL_UPDATE" then
    ...
    if progress == total then
      ...
      -- Same private-core compatibility as above: once multisell reports done,
      -- do not wait for the owned auction list to refresh.
      FrameUtil.UnregisterFrameForEvents(self, NEW_AUCTION_EVENTS)
      FrameUtil.UnregisterFrameForEvents(self, OWNER_LIST_EVENTS)
      self.waitingForNewAuction = false
      self.waitingForOwnerAuctionsUpdate = false
